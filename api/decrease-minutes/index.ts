...
type DecreaseMinutesRequest = {
  organizationId?: string;
  minutes?: number;
};

export async function decreaseMinutes(
  request: HttpRequest,
  context: InvocationContext
): Promise<HttpResponseInit> {
  context.log("decrease-minutes request received");

  try {
    const auth = await authenticateRequest(request);
    const isAdmin = userIsAdmin(auth);
    const body = (await request.json().catch(() => ({}))) as DecreaseMinutesRequest;
    ...
    if (typeof minutesRaw !== "number" || !Number.isFinite(minutesRaw) || minutesRaw <= 0) {
      throw new AuthorizationError("minutes must be a positive number", 400);
    }
    const decrement = Math.ceil(minutesRaw);
    ...
    if ((org.remainingMinutes ?? 0) < decrement) {
      throw new AuthorizationError("Remaining minutes exceeded", 403);
    }
    ...
    const updated = await patchItem<Organization>(
      ...
      targetOrgId,
      [
        { op: "incr", path: "/remainingMinutes", value: -decrement },
    ...
    context.error("decrease-minutes error", e);
    return {
      status,
...
app.http("decrease-minutes", {
  methods: ["POST"],
  authLevel: "anonymous",
  route: "organizations/decrease-minutes",
  handler: decreaseMinutes,
});
